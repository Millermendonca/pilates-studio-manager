...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    if (quickMenuOpen || adminMenuOpen) {
      document.addEventListener('click', handleClickOutside);
    }
    return () => document.removeEventListener('click', handleClickOutside);
  }, [quickMenuOpen, adminMenuOpen]);
    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/80 shadow-xs transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 gap-2 sm:gap-4">
          ...
          {/* Navegação Desktop Principal (Oculta na tela de login do admin) */}
          {!isAdminLoginPage && (
          ...
          <div className="flex items-center space-x-2 sm:space-x-2.5">
            ...
            {/* Menu do Gestor Autenticado / Segurança */}
            {adminUser && (
              <div className="relative" id="admin-user-menu">
                <button
                  type="button"
                  onClick={() => setAdminMenuOpen(!adminMenuOpen)}
                  className="flex items-center space-x-2 px-2.5 py-1.5 bg-slate-900 hover:bg-black text-white rounded-xl shadow-sm transition-all"
                  aria-label="Perfil do Administrador"
                >
                  <div className="w-6 h-6 rounded-lg bg-emerald-500 flex items-center justify-center text-white text-[11px] font-black shrink-0">
                ...
                {adminMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="px-3 py-2 border-b border-slate-100 mb-1">
                      <p className="text-[11px] font-bold text-slate-900 truncate">{adminUser.name}</p>
                      <p className="text-[10px] text-slate-400 font-mono">@{adminUser.username}</p>
                    </div>

                    <Link
                      href="/configuracoes"
                      className="flex items-center space-x-2 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-pilates-700 transition-colors"
                    >
                      <KeyRound className="w-4 h-4 text-emerald-600" />
                      <span>Trocar Senha & Segurança</span>
                    </Link>
                    ...
                    <button
                      ...
                      className="w-full flex items-center space-x-2 px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 transition-colors text-left"
                    >
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="px-3 py-1.5">
                    <div className="flex items-center space-x-2 text-[11px] text-slate-500">
                      ...
                      <span>Bcrypt 12 + JWT + Lockout</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      {mobileMenuOpen && !isAdminLoginPage && (
          ...
          {adminUser && (
            <div className="pt-2 border-t border-slate-100">
              <button
                type="button"
                onClick={handleLogout}
                className="w-full flex items-center justify-center space-x-2 py-2.5 px-4 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-bold border border-rose-200 transition-colors"
              >
                <LogOut className="w-4 h-4" />
                <span>Sair do Sistema (Logout)</span>
              </button>
